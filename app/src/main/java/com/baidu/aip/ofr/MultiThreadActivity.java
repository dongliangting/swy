...
public class MultiThreadActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext=this;
        singleThreadExecutor = Executors.newFixedThreadPool(MAX_THREAD_NUM);

        PreferencesUtil.putInt(GlobalFaceTypeModel.TYPE_THREAD, GlobalFaceTypeModel.MULTITHREAD);
        type=PreferencesUtil.getInt(GlobalFaceTypeModel.TYPE_THREAD,GlobalFaceTypeModel.SINGLETHREAD);
    ...
    private void initSDK() {
        ...
        faceTracker = getFaceTracker(mContext);
        faceTracker1 = getFaceTracker(mContext);

        if (type==GlobalFaceTypeModel.MULTITHREAD){
            // 人脸可见光离线识别模型初始化，通过index调用FaceSDK.extractFeature方法提取人脸特征值
            FaceSDK.recognizeModelInit(this, MAX_THREAD_NUM, FaceSDK.RecognizeType.RECOGNIZE_LIVE);
            ...
            FaceSDK.livenessSilentInit(this, FaceSDK.LivenessTypeId.LIVEID_VIS, MAX_THREAD_NUM);
    ...
    private void testMethrod() {
        new Thread(new Runnable() {
            @Override
            public void run() {
                ARGBImg argbImg = getData(mContext, "pic/testRegPic" + 0 + ".png");
...
//                }
    ...
    private void stratFaceWork(int index, FaceTracker faceTracker, ARGBImg argbImg) {
        ...
        if (argbImg != null) {
            ...
            Log.e(TAG,"Thread"+index+"人脸检测的时间"+rgbDetectDuration);
            ...
            if (faceInfos != null && faceInfos.length > 0) {
                ...
                Log.e(TAG,"Thread"+index+"特征抽取的时间"+extractFeatureDuration);
                ...
                Log.e(TAG,"Thread"+index+"活体检测的时间"+livenessDuration);
...
//    }
    ...
    protected void onDestroy() {
        super.onDestroy();
        PreferencesUtil.putInt(GlobalFaceTypeModel.TYPE_THREAD, GlobalFaceTypeModel.SINGLETHREAD);
    }
}

...
public class VoiceManager {
    ...
    @Inject
    public VoiceManager(AIUIWrapper wrapper, ChatRepo chatRepo, AIUIConfigCenter configManager, TTSManager ttsManager, AIUIPlayer aiuiPlayer) {
        mChatRepo = chatRepo;
        mAgent = wrapper;
...
//                                semanticResult.toString().getBytes(), null, rspTime);   //dlt

                        Log.e("lt-----",semanticResult.toString());
                        JSONObject jsonObjectBean = new JSONObject(semanticResult.toString());
                        JSONObject jsonAnswer =  jsonObjectBean.getJSONObject("answer");
                        JSONObject jsonQuestion = jsonAnswer.getJSONObject("question");
                        String myQuestion = jsonQuestion.getString("question");
                        String strAnswer = jsonAnswer.getString("text");
                        EventBus.getDefault().post(new MessageEvent(myQuestion,strAnswer,semanticResult.toString()));

//                        addChatMessage(rawMessage);    //dlt
                    }
